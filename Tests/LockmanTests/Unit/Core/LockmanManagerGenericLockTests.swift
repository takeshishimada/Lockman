  ...
  func testGenericLock_DifferentReturnTypes_Success() async throws {
    ...
    await LockmanManager.withTestContainer(container) {
      ...
      let boolResult: Bool = LockmanManager.lock(
        ...
        boundaryId: boundaryId,
        unlockOption: nil,
        onSuccess: { _, unlock in unlock(); return true },
        onSuccessWithPrecedingCancellation: { _, _, unlock in unlock(); return false },
        onCancel: { _, _ in false },
        onError: { _, _ in false }
      )
      ...
      let intResult: Int = LockmanManager.lock(
        ...
        boundaryId: boundaryId,
        unlockOption: nil,
        onSuccess: { _, unlock in unlock(); return 123 },
        onSuccessWithPrecedingCancellation: { _, _, unlock in unlock(); return 0 },
        onCancel: { _, _ in 0 },
        onError: { _, _ in 0 }
      )
