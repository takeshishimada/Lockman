...
final class LockmanRegistrationErrorTests: XCTestCase {
  ...
  func testLockmanRegistrationErrorDescription() {
    ...
    let expectedAlreadyRegDesc = "Strategy 'TestStrategy' is already registered. Each strategy type can only be registered once."
    ...
    let expectedNotRegDesc = "Strategy 'UnknownStrategy' is not registered. Please register the strategy before attempting to resolve it."
    ...
    let expectedEmptyAlreadyReg = "Strategy '' is already registered. Each strategy type can only be registered once."
    ...
    let expectedEmptyNotReg = "Strategy '' is not registered. Please register the strategy before attempting to resolve it."
  ...
  func testLockmanRegistrationErrorFailureReason() {
    ...
    let alreadyRegError = LockmanRegistrationError.strategyAlreadyRegistered("TestStrategy")
    let expectedAlreadyRegReason = "The container enforces unique strategy type registration to prevent conflicts and ensure deterministic behavior."
    XCTAssertEqual(alreadyRegError.failureReason, expectedAlreadyRegReason)
    
    // Test failureReason for .strategyNotRegistered
    let notRegError = LockmanRegistrationError.strategyNotRegistered("UnknownStrategy")
    let expectedNotRegReason = "Strategy resolution requires that the strategy type has been previously registered in the container."
  ...
  func testLockmanRegistrationErrorRecoverySuggestion() {
    ...
    let alreadyRegError = LockmanRegistrationError.strategyAlreadyRegistered("TestStrategy")
    let expectedAlreadyRegSuggestion = "Check if 'TestStrategy' is being registered multiple times. Use container.isRegistered(_:) to check before registration, or ensure registration happens only once during app startup."
    ...
    let notRegError = LockmanRegistrationError.strategyNotRegistered("UnknownStrategy")
    let expectedNotRegSuggestion = "Add 'try LockmanManager.container.register(UnknownStrategy.shared)' to your app startup code, or verify that registration is happening before this resolution attempt."
    ...
    let expectedCustomSuggestion = "Check if 'MyCustomStrategy' is being registered multiple times. Use container.isRegistered(_:) to check before registration, or ensure registration happens only once during app startup."
  ...
  func testLockmanRegistrationErrorHelpAnchor() {
    ...
    let differentNameError1 = LockmanRegistrationError.strategyAlreadyRegistered("DifferentStrategy")
  ...
  func testLockmanRegistrationErrorSendableConformance() async {
    ...
    await withTaskGroup(of: String.self) { group in
      ...
      for result in results {
        ...
        XCTAssertTrue(result.contains("registered") || result.contains("container") || result.contains("No reason"))
      }
    }
  }
  ...
  func testLockmanRegistrationErrorWithSpecialCharacters() {
    ...
    for strategyName in specialCases {
      ...
      XCTAssertTrue(alreadyRegError.errorDescription?.contains(strategyName) == true,
                   "Error description should contain strategy name: '\(strategyName)'")
      XCTAssertTrue(notRegError.errorDescription?.contains(strategyName) == true,
                   "Error description should contain strategy name: '\(strategyName)'")
      
      // Verify recovery suggestions contain the strategy name
      XCTAssertTrue(alreadyRegError.recoverySuggestion?.contains(strategyName) == true,
                   "Recovery suggestion should contain strategy name: '\(strategyName)'")
      XCTAssertTrue(notRegError.recoverySuggestion?.contains(strategyName) == true,
                   "Recovery suggestion should contain strategy name: '\(strategyName)'")
    }
  }
  ...
  private func classifyRegistrationError(_ error: LockmanRegistrationError) -> (type: String, strategyName: String) {
    switch error {
    case .strategyAlreadyRegistered(let name):
