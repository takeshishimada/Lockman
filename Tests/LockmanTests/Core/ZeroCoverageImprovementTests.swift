...
final class ZeroCoverageImprovementTests: XCTestCase {
  ...
  func testLockmanCancellationErrorInstantiation() {
    ...
    XCTAssertNotNil(error.errorDescription)
    XCTAssertNotNil(error.failureReason)
    XCTAssertEqual(String(describing: error.boundaryId), String(describing: AnyLockmanBoundaryId(boundaryId)))
  }
  ...
  func testLockmanResultEquatableConformance() {
    ...
    let differentSuccessWithCancel = LockmanResult.successWithPrecedingCancellation(error: differentPrecedingError)
