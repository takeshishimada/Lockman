...
final class LockmanDebugFormattersTests: XCTestCase {
  ...
  func testFormatStrategyNameWithShortNames() {
    ...
    XCTAssertEqual(
      LockmanManager.debug.formatStrategyName("LockmanConcurrencyLimitedStrategy", options: options),
      "ConcurrencyLimited"
    )
  }
  ...
  func testFormatBoundaryIdSimplification() {
    ...
    let wrappedId = "AnyLockmanBoundaryId(base: AnyHashable(Strategies.SingleExecutionStrategyFeature.CancelID.userAction))"
    XCTAssertEqual(
  ...
  func testFormatBoundaryIdComplexAnyHashableWrapper() {
    ...
    let complexWrappedId = "AnyLockmanBoundaryId(base: AnyHashable(Module.SubModule.Feature.CancelID.complexAction))"
  ...
  func testPrintCurrentLocksWithContentWidthExceedingLimits() {
    ...
    let strategy = LockmanSingleExecutionStrategy()
    let veryLongBoundaryId = "VeryLongBoundaryIdThatShouldExceedTheMaximumWidthLimitSettingOfTwentyFiveCharacters"
    let veryLongActionId = "VeryLongActionIdThatExceedsTheThirtySixCharacterLimitAndShouldBeTruncatedByTheFormatting"
